...
@property (nonatomic) NSUInteger indexOfSecondComponent;
...
- (void)setup
{
    ...
    UIColor *color1 = [UIColor colorWithRed:126.0/255.0f green:140.0/255.0f blue:1.0/255.0f alpha:1.0f];
    self.toolbar.backgroundColor = color1;//[UIColor blackColor];
    ...
    self.doneButton.titleLabel.font = [UIFont fontWithName:@"Lato-Regular" size:20];
    self.cancelButton.titleLabel.font = [UIFont fontWithName:@"Lato-Regular" size:20];
    
//    if ([Localize.currentLanguage isEqualToString:@"ar"]){
//        [self.doneButton setTitle:@"تم" forState:UIControlStateNormal];
//        [self.cancelButton setTitle:@"إلغاء" forState:UIControlStateNormal];
//    }else{
...
- (void)setPickerType:(PickerType)pickerType
{
    ...
    switch (pickerType)
    {
        case DateAndTimePickerType:
        {
            ...
            [self.datePicker setLocale:locale]; // for 24 hours format
...
- (IBAction)doneButtonAction:(UIButton *)sender
{
        ...
        self.pickerType == DatePickerType)
    {
        ...
        if (self.pickerType == CustomPickerType) {
            [self.delegate didSelectItemAtIndex:self.indexOfSelectedRowInPicker withFormat:self.indexOfSecondComponent withString:self.selectedString];
        }
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view
{
    if (component == 1) {
        ...
        if (!tView) {
            ...
            tView.font = [UIFont fontWithName:@"Lato-Regular" size:20];
            UIColor *color1 = [UIColor colorWithRed:126.0/255.0f green:140.0/255.0f blue:1.0/255.0f alpha:1.0f];
            tView.textColor = color1;//[UIColor blackColor];
            tView.textAlignment = NSTextAlignmentCenter;
        }
        
        // * Supported formats
        if ([self.arrayAMPM.firstObject isKindOfClass:[NSString class]]) {
            tView.text = self.arrayAMPM[row];
        } else if ([self.arrayAMPM.firstObject isKindOfClass:[NSNumber class]]) {
            tView.text = [self.arrayAMPM[row] stringValue];
        }
        ...
        if ([self.dataSourceForCustomPickerType.firstObject isKindOfClass:[NSString class]]) {
            ...
            self.selectedString = self.dataSourceForCustomPickerType[row];
            ...
            self.selectedString = [self.dataSourceForCustomPickerType[row] stringValue];
        }
